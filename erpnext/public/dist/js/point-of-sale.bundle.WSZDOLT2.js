...
`);
  ...
  erpnext.PointOfSale.Payment = class {
    ...
    on_numpad_clicked($btn, from_numpad = true) {
      ...
      const precision2 = frappe.sys_defaults.currency_precision || number_format_details.precision;
      this.numpad_value = "0";
      if (this.selected_mode.get_value()) {
        this.numpad_value = (this.selected_mode.get_value() * 10 ** precision2).toFixed(0).toString();
      }
      ...
      if (button_value === "delete" || button_value === "Backspace") {
        ...
        this.numpad_value = Number(this.numpad_value) >= 0 ? this.numpad_value : `${this.numpad_value * -1}`;
      } else if (button_value === "-") {
        this.numpad_value = Number(this.numpad_value) <= 0 ? this.numpad_value : `${this.numpad_value * -1}`;
      ...
      this.selected_mode.set_value(this.numpad_value / 10 ** precision2);
    ...
    bind_events() {
      ...
      this.$payment_modes.on("click", ".mode-of-payment", function(e) {
        ...
        me.hide_zero_amount();
        if (((_a = me.selected_mode) == null ? void 0 : _a._label) === ((_b = me[`${mode}_control`]) == null ? void 0 : _b._label)) {
      ...
      this.$component.on("click", ".submit-order-btn", () => {
        ...
        if (!items.length || paid_amount == 0 && doc.additional_discount_percentage != 100 && this.allow_partial_payment === 0) {
    ...
    hide_zero_amount() {
      ...
      for (let i = 0; i < payment_methods.length; i++) {
        ...
        if (((_a = this[`${mode}_control`]) == null ? void 0 : _a.value) === 0) {
    ...
    render_payment_mode_dom() {
      ...
      this.$payment_modes.html(
        `${payments.map((p, i) => {
          ...
          const amount = p.mode_of_payment === ((_a = this.selected_mode) == null ? void 0 : _a._label) || p.amount !== 0 ? format_currency(p.amount, currency) : "";
          return `
    ...
    validate_reqd_invoice_fields() {
      if (this.invoice_fields.length === 0)
        return true;
      const doc = this.events.get_frm().doc;
  ...
  erpnext.PointOfSale.PastOrderSummary = class {
    ...
    get_condition_btn_map(after_submission) {
      ...
      return [
        ...
        {
          condition: !this.doc.is_return && this.doc.docstatus === 1 && !["Partly Paid", "Overdue", "Unpaid"].includes(this.doc.status),
  ...
  erpnext.PointOfSale.Controller = class {
    ...
    async fetch_invoice_fields() {
      ...
      const pos_settings = await frappe.db.get_doc("POS Settings", void 0);
    ...
    setup_listener_for_pos_closing() {
      frappe.realtime.on(`poe_${this.pos_opening}`, (data) => {
        ...
        if (data && route == "point-of-sale") {
          frappe.dom.freeze();
          const title = data.operation === "Closed" ? __("POS Closed") : __("POS Opening Entry Cancelled");
          const msg = data.operation === "Closed" ? __("POS has been closed at {0}. Please refresh the page.", [
            frappe.datetime.str_to_user((_a = data.doc) == null ? void 0 : _a.creation).bold()
...
`);
    ...
    toggle_recent_order_list(show) {
      this.frm.doc.docstatus === 1 ? this.toggle_submitted_invoice_summary(!show) : this.toggle_components(!show);
...
//# sourceMappingURL=point-of-sale.bundle.WSZDOLT2.js.map
